...
├── admin/            # 管理后台 (Vue 3, 端口 3003, 生产构建 base=/admin/)
├── web/              # Web 前端 (Vue 3, 端口 4000)
...
```bash
mysql -u root -p -e "CREATE DATABASE IF NOT EXISTS fang123 DEFAULT CHARSET utf8mb4 COLLATE utf8mb4_unicode_ci;"
mysql -u root -p fang123 < backend/src/main/resources/db/schema.sql
```
...
> 前端通过 Vite proxy 将 `/api` 请求转发到后端 `localhost:8090`。
...
| **房产模块** | | | |
| 楼盘列表 | GET | `/api/admin/loupans` | 分页+搜索（名称/区/板块/开发商） |
| 楼盘详情 | GET | `/api/admin/loupans/{id}` | 单条 |
| 创建楼盘 | POST | `/api/admin/loupans` | 新增 |
| 更新楼盘 | PUT | `/api/admin/loupans/{id}` | 编辑 |
| 删除楼盘 | DELETE | `/api/admin/loupans/{id}` | 逻辑删除 |
| 地块列表 | GET | `/api/admin/tupai-lands` | 分页+搜索 |
| 创建/更新/删除 | POST/PUT/DELETE | `/api/admin/tupai-lands[/{id}]` | CRUD |
| 户型列表 | GET | `/api/admin/huxings` | 分页+搜索+按楼盘过滤 |
...
java -jar target/fang123-backend-*.jar --spring.profiles.active=prod --server.port=8090
...
server {
    ...
    # 上传素材可达 500MB（与后端 max-file-size 保持一致），需提高 body 上限并延长超时
    ...
    # 管理后台 (base=/admin/)
    location /admin/ {
        alias /www/wwwroot/fang123/admin/dist/;
        try_files $uri $uri/ /admin/index.html;
    }

    # Web 前端
    location / {
        root /www/wwwroot/fang123/web/dist/;
...
| `loupan_yfyj` | 一房一价房源表（楼栋-楼层-房号/备案价/销售价/房源状态/关联楼盘和户型） |
